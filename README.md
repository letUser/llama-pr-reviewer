...
- **Incremental mode**: prior reviewed SHA inferred from the bot's last comment timestamp. Diff base is **always** the merge-base with the target branch (PR net change), never the prior SHA directly — so a `Merge branch development` can't drag other tickets in. Incremental keeps that base and only narrows it by pathspec to files the PR's own non-merge commits touched since the prior review; base-merge spillover is excluded. No PR-own changes since prior review → re-approve via skip path, no model call.
